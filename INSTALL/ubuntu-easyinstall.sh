...
if [ "$mysqlagreeData" = "y" ]; then
    mysql -e "source backend/sql/user.sql" || true
    ...
    if [ "$mysqlDefaultData" = "y" ]; then
        ...
        rm backend/sql/default_user.sql || true
        echo "USE ccio;INSERT INTO Users (\`ke\`,\`uid\`,\`auth\`,\`mail\`,\`pass\`,\`details\`) VALUES (\"$groupKey\",\"$userID\",\"$apiKey\",\"$userEmail\",\"$userPasswordMD5\",\"$userDetails\");INSERT INTO API (\`code\`,\`ke\`,\`uid\`,\`ip\`,\`details\`) VALUES (\"$apiKey\",\"$groupKey\",\"$userID\",\"$apiIP\",\"$apiDetails\");" > "backend/sql/default_user.sql"
        mysql -u "$sqluser" -p"$sqlpass" --database ccio -e "source backend/sql/default_user.sql" > "INSTALL/log.txt"
        echo "====================================="
