...
if [ "$mysqlagreeData" = "y" ] || [ "$mysqlagreeData" = "Y" ]; then
    ...
    sudo mysql -u "$sqluser" -p"$sqlpass" -e "source backend/sql/user.sql" || true
fi
echo "============="
...
node backend/tools/modifyConfiguration.js addToConfig="{\"cron\":{\"key\":\"$(head -c 64 < /dev/urandom | sha256sum | awk '{print substr($1,1,60)}')\"}}"
