...
const initialize = (config,lang) => {
    ...
    const createXenocipherSocketConnection = (connectionId) => {
    ...
    function startBridge(noLog){
        ...
        // const masterConnectionToMachine = createXenocipherSocketConnection()
        ...
        connectionToP2PServer.on('wsInit',(rawRequest) => {
            ...
            const clientConnectionToMachine = createXenocipherSocketConnection(rawRequest.cnid)
        ...
        ([
          ...
          'super',
            ...
            connectionToP2PServer.on(target,(data) => {
                ...
                if(data.f === 'init'){
                    clientConnectionToMachine = createXenocipherSocketConnection(data.cnid)
                    clientConnectionToMachine.on('connect', () => {
        ...
        config.workerStreamOutHandlers.forEach((target) => {
            connectionToP2PServer.on(target,(initData) => {
                if(connectedUserWebSockets[initData.auth]){
                    const clientConnectionToMachine = createXenocipherSocketConnection(initData.auth + initData.ke + initData.id)
                    clientConnectionToMachine.on('connect', () => {
