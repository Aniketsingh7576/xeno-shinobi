...
module.exports = (s,config,lang) => {
    ...
    const createStreamChannel = function(e,number,channel){
        ...
        switch(streamType){
            ...
            break;
            case'mp4':
                streamFlags.push(`-f mp4 -movflags +frag_keyframe+empty_moov+default_base_moof -metadata title="Poseidon Stream from Xenocipher" -reset_timestamps 1 pipe:${number}`)
            break;
            case'flv':
    ...
    const buildMainStream = function(e){
		...
		if (!e.details.hwaccel_format) {
            ...
            switch(streamType){
                case'mp4':
                    streamFlags.push('-f mp4 -movflags +frag_keyframe+empty_moov+default_base_moof -metadata title="Poseidon Stream from Xenocipher" -reset_timestamps 1 pipe:1')
                break;
                case'flv':
