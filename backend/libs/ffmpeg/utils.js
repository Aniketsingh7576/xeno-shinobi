    ...
    const checkForUnix = function(){
        ...
        if(s.isWin === false){
            if (fs.existsSync('/usr/bin/ffmpeg')) {
                ...
                response.ok = true
                config.ffmpegDir = '/opt/homebrew/bin/ffmpeg'
            }else{
                try{
                    const pathFfmpeg = execSync('command -v ffmpeg').toString().trim()
                    if(pathFfmpeg && fs.existsSync(pathFfmpeg)){
                        response.ok = true
                        config.ffmpegDir = pathFfmpeg
                    }
                }catch(err){}
            }
        }
