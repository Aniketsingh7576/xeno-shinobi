...
module.exports = async (s,config,lang,app,io,currentUse) => {
    const { fetchDownloadAndWrite } = require('../basic/utils.js')(s.mainDirectory,config)
    const {
    ...
    const modulesBasePath = s.mainDirectory + '/plugins/'
    ...
    const installModule = (name) => {
        return new Promise((resolve, reject) => {
            if(!runningInstallProcesses[name]){
                ...
                var installProcess
                const tempRunPath = `${s.mainDirectory}/plugin-install-${name}.sh`
