                ...
                switch(d.f){
                    ...
                    case'system':
                        switch(d.ff){
                            case'update':
                                s.ffmpegKill()
                                s.systemLog('Xenocipher ordered to update',{
                                    by:cn.mail,
                                    ip:cn.ip
                                })
                            ...
                            case'restart':
                                ...
                                if(d.check('system')){
                                    s.systemLog('Xenocipher ordered to restart',{by:cn.mail,ip:cn.ip})
                                    s.ffmpegKill()
                                    exec('pm2 restart '+s.mainDirectory+'/camera.js')
                                }
                                if(d.check('cron')){
                                    s.systemLog('Xenocipher CRON ordered to restart',{by:cn.mail,ip:cn.ip})
