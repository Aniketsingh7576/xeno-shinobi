...
module.exports = async function(s,config,lang){
    if(config.webDavUpload){
        try{
            ...
            console.error('Could not start WebDAV Uploader, please run "npm install webdav-fs" inside the Xenocipher folder.')
            console.error('The webdav-fs Module is known to have possible vulnerabilities. Due to the nature of the vulnerability you should be unaffected unless Xenocipher is public facing. Use at your own risk.')
        }
    }else{
        try{
            const webdav = await import("webdav-fs");
