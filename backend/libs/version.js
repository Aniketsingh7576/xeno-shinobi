...
module.exports = function(s,config,lang,app,io){
    const execOptions = { cwd: s.repoDirectory || s.mainDirectory }
    function getLastUsedCommits(numberOfCommits = 3){
        return new Promise((resolve) => {
            exec(`git log -${numberOfCommits} --pretty=format:"%H"`, execOptions, function(err, response){
                try{
                    if(err){
                        s.debugLog('Git repository metadata is unavailable.')
                        resolve([])
                    }else if(response){
    ...
    var getRepositoryCommitId = function(callback){
        exec(`git rev-parse HEAD`, execOptions, function(err,response){
            if(response){
