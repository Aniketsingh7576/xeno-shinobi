...
module.exports = function(s,config,lang,io){
    ...
    //SSL options
    var wellKnownDirectory = s.frontendDirectory + '/.well-known'
    ...
    if(sslInfo && sslInfo.key && sslInfo.cert){
        try{
            ...
            serverHTTPS.listen(sslInfo.port,config.bindip,function(){
                console.log('SSL '+lang.Xenocipher+' : SSL Web Server Listening on '+sslInfo.port);
            });
    ...
    server.listen(config.port,config.bindip,function(){
        console.log(lang.Xenocipher+' : Web Server Listening on '+config.port);
    });
