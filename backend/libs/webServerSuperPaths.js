...
module.exports = function(s,config,lang,app){
    ...
    */
    ...
    /**
    * API : Superuser : Restart Xenocipher
    */
    app.all(config.webPaths.superApiPrefix+':auth/system/restart/:script', function (req,res){
        s.superAuth(req.params,function(resp){
            ...
            if(check('system')){
                s.systemLog('Xenocipher ordered to restart',{by:resp.$user.mail,ip:resp.ip})
                s.ffmpegKill()
                endData.systemOuput = execSync('pm2 restart '+s.mainDirectory+'/camera.js')
            }
            if(check('cron')){
                s.systemLog('Xenocipher CRON ordered to restart',{by:resp.$user.mail,ip:resp.ip})
