...
| [**scaling-comparison.html**](scaling-comparison.html) | **Visual side-by-side** of the current master-funnel design vs. the industry standalone+directory pattern — the diagrams that make the bottleneck (and the fix) obvious at a glance. Open in any browser. |
| [**06 — Architecture Decision**](06-Architecture-Decision.md) | The go/no-go: an exhaustive adversarial failure analysis (28 agents, every failure type × both architectures, verified against code) concluding **move to standalone** — with the verified catastrophic cluster failures, the standalone requirements, and why moving turns *off* the riskiest code rather than adding new risk. |
| [**07 — Standalone Rollout Plan**](07-Standalone-Rollout-Plan.md) | The concrete how-to: grow-as-you-go, RAID-level durability. Phase 0 (the one required NAS mount-health code safeguard), Phase 1 (stand up one server — exact config keys, NAS/RAID setup, storage-limit gotcha, validation), Phase 2 (add servers + thin directory), Phase 3 (thousands). Plus a readiness checklist. |
