...
var keyShortcuts = {}
var shinobiUtils = new XenocipherUtils()
...
function dashboardOptions(r,rr,rrr){
    if(!rrr){rrr={};};if(typeof rrr === 'string'){rrr={n:rrr}};if(!rrr.n){rrr.n='XenocipherOptions_'+location.host+'_'+$user.ke+$user.uid}
...
//     localStorage.setItem(`Xenocipher-Tab-States-${location.origin}`,JSON.stringify(loadedPages))
// }
//
// function loadTabStates(){
//     var tabStates = JSON.parse(localStorage.getItem(`Xenocipher-Tab-States-${location.origin}`)) || {}
