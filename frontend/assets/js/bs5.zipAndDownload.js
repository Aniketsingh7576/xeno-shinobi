$(document).ready(function(){
    ...
    var zipVideosAndDownload = function(videos,onSuccess){
        var zip = new JSZip();
        var zipFileName = `XenocipherVideos_${moment(new Date()).format('YYYY-MM-DDTHH-mm-ss')}.zip`
        var foldersCreated = {}
        var downloadBars = {}
        var progressBarHtml = []
        var videosCopy = Object.assign(videos,{})
        $.each(videosCopy,function(n,video){
            var fileZipName = `${video.filename}`
    ...
    function zipVideosAndDownloadWithConfirm(selectedVideos){
        ...
        $.confirm.create({
            title: lang['Zip and Download'],
            body: lang.ExportSelectedVideosMsg,
            clickOptions: {
                title: lang['Zip and Download'],
    ...
    window.zipVideosAndDownload = zipVideosAndDownload
    window.zipVideosAndDownloadWithConfirm = zipVideosAndDownloadWithConfirm
})
